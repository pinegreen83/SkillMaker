...
이 문서는 코드 작성·수정 시 준수할 규칙입니다. 모든 기존 코드가 이미 이 규칙을 충족한다는 뜻은 아닙니다. 2026-09-07 검토에서 확인한 미준수와 연결 누락은 [구현 현황](IMPLEMENTATION_STATUS.md), 실제 책임과 호출 관계는 [프로젝트 구조](PROJECT_MAP.md)에 기록합니다. 기능 범위와 제품 기획은 여기서 확정하지 않습니다.
...
- 저장소 용량 관리를 위해 `Content/*`는 Git 추적 대상에서 제외한다. 로컬에서 수정한 `.uasset`과 `.umap`을 강제 추가하거나 `.gitignore` 예외로 등록하지 않는다.
...
- WidgetSwitcher를 인덱스 기반 상태 열거형으로 제어할 때는 열거형과 블루프린트 자식 순서를 맞춘다. 자식 순서가 바뀔 수 있는 화면은 패널을 직접 바인딩해 `SetActiveWidget`으로 전환한다.
- 블루프린트가 위젯 계층과 배치를 소유하는 경우 C++은 `BindWidget`으로 참조하고 런타임에 부모를 바꾸지 않는다. 필요한 계층이 다르면 WBP를 수정한 뒤 C++에서 계층과 타입을 검증한다.
- 위젯 초기화 중 GameInstance나 서브시스템을 조회하는 코드는 디자인 타임과 에셋 검증 월드를 먼저 제외하고, 실제 게임 월드의 연결 누락만 오류로 기록한다.
- UI에서 수정 중인 데이터와 최종 저장 데이터의 소유자를 분리한다.
  - 편집 중 원본: 제작 HUD의 `CurrentEditingSkill`
  - 선택 위젯: 원본을 소유하지 않고 현재 값을 표시하며 선택 결과를 델리게이트로 전달
...
- SkillMaker UI는 선택 결과와 수치 값을 HUD가 소유한 `FSKSkillData`에 구성한다. 상세 위젯은 이벤트 순간 HUD 최신값의 지역 사본에서 필요한 필드만 바꾸고 즉시 HUD로 전달한다.
...
- 무기와 애니메이션 호환성은 `Weapon` 계층 Gameplay Tag로 표현하며 무기 행과 애니메이션 행의 호환 태그를 비워두지 않는다.
- 데이터 규모가 늘어나기 전에 에디터 데이터 검증 또는 전용 검증 도구로 빈 태그, `Weapon` 계층 밖의 태그와 필수 몽타주 누락을 행 단위로 보고할 수 있게 한다.
- 스킬 속성은 `Element` 계층 Gameplay Tag 하나로 저장하고, 이펙트·사운드가 지원하는 속성은 `FGameplayTagContainer`로 표현한다. 지원 속성이 비어 있는 기존 리소스 행은 마이그레이션 전까지 모든 속성과 호환되는 것으로 취급한다.

## 소프트 참조와 패키징 규칙

- 데이터 행과 저장 스킬은 몽타주·썸네일·발사체처럼 큰 리소스를 하드 참조로 일괄 보유하지 않고 소프트 참조로 유지한다.
- 목록 구성 중 몽타주와 발사체 클래스를 일괄 동기 로드하지 않는다. 썸네일은 소프트 텍스처 API를 사용한다.
- 현재 코드는 몽타주 노티파이 조회와 스킬 실행, 트리거의 발사체 생성 지점에서 필요한 리소스를 동기 로드한다. 데이터가 늘어나면 스킬 로드 단계의 선행 로드와 캐시 정책을 먼저 설계한다.
- 문자열 경로로 데이터 테이블을 찾는 것만으로 패키징 포함이 보장된다고 가정하지 않는다. 패키징 검증에서 데이터 테이블과 내부 소프트 참조 에셋이 쿠킹됐는지 확인하고 필요하면 Asset Manager 또는 명시적인 쿠킹 설정으로 포함한다.
- 현재 목록 규모에서는 스크롤 UI를 사용할 수 있다. 규모가 커지면 페이징, 리스트 가상화 또는 지연 로드를 검토한다.
...
아래는 준수해야 할 규칙이며 현재 구현의 완료 설명이 아니다. 발사체의 직접 피해는 연결됐으며 상태이상 타이머·이동 복구·멀티캐스트 처리와 멀티플레이 권한의 차이는 구현 현황 COMBAT-02, NET-01에 기록한다.
...
- 버튼 델리게이트 중복 바인딩 여부
- WidgetSwitcher 자식 순서 또는 직접 바인딩한 탭 패널과 실제 활성 위젯
