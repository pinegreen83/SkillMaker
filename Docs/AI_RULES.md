...
- 스킬 생성·편집, SaveGame 저장, 훈련장 스킬 선택·슬롯 배치·입력 실행·매핑 현황 UI, 발사체·전투 컴포넌트가 있다. 발사체의 전투 정보·공격자 전달과 공통 전투 컴포넌트 피해 적용은 제작 미리보기와 훈련장에서 실행 확인했으며 상태이상 등 남은 범위는 구현 현황을 확인한다.
